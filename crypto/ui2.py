...
def dec(users,id,input_file):
    #print("u have clicked the upload file ")
    ...
    # Get input file
    #input_file = input("Enter nameinput image file: ")
    BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))  
    STATIC_DIR = os.path.join(BASE_DIR, 'static')
    UPLOADS_DIR=os.path.join(BASE_DIR, 'uploads')
    dec_file=STATIC_DIR+"/"+input_file+"_dec.jpg"
    input_file= UPLOADS_DIR+"/"+input_file+".jpg"
    ...
    try:
        ...
        success = des.decrypt_image( input_file,dec_file)
        if success:
            print(f"Decryption complete. Output saved to {dec_file}")
...
# dec(["user1@example.com", "user2@example.com", "user3@example.com"],"alice@example.com","img_enc")